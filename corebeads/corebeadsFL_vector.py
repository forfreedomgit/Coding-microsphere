...
X_ = np.array(data[['FSFL_vector', 'AFL_vector']])
y_ = np.array(data['corebeadsFL_vector'])

# Normalize data
X_ = (X_ - np.mean(X_, axis=0)) / np.std(X_, axis=0)

# y_ = (y_ - np.mean(X_, axis=0)) / np.std(y_, axis=0)
...
X_ = data[['FSFL_vector', 'AFL_vector']].values[:, 0]
Y_ = data[['FSFL_vector', 'AFL_vector']].values[:, 1]

Z = predicate_results

rm_and_lstp_price = ax.plot_trisurf(X_, Y_, Z, color='red')

ax.set_xlabel('FSFL_vector')
ax.set_ylabel('AFL_vector')
ax.set_zlabel('Predicated-corebeadsFL_vector')
plt.show()
