...
dataPath = 'E:/working/corebeads/corebeads.csv'
...
X_ = np.array(data[['FSFL_A', 'AFL_FS']])
y_ = np.array(data['corebeadsFL_A'])
...
X_ = data[['FSFL_A', 'AFL_FS']].values[:, 0]
Y_ = data[['FSFL_A', 'AFL_FS']].values[:, 1]

Z = predicate_results

rm_and_lstp_price = ax.plot_trisurf(X_, Y_, Z, color='red')

ax.set_xlabel('FSFL_A')
ax.set_ylabel('AFL_FS')
ax.set_zlabel('Predicated-corebeadsFL_A')
plt.show()
